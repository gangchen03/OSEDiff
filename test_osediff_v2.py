...
def inference_in_parallel(index, all_image_names):
    ...
    num_images_per_process = len(all_image_names) // world_size + 1
    start_index = rank * num_images_per_process
    end_index = (rank + 1) * num_images_per_process
    if rank == world_size - 1:
        end_index = len(all_image_names)
    ...
    for image_name in my_image_names:
        ...
        with torch.no_grad():
            time0 = time.time()
            lq = lq*2-1
            output_image = model(lq.to(device), prompt=validation_prompt)
            time1 = time.time()
            print(f"model total time is: {time1 - time0}")
            #print(output_image[0])
            #print(f"print time is: {time.time() - time1}")
            output_im = output_image.to('cpu').to(dtype = torch.float32) * 0.5 + 0.5
            ...
            print(f"time is: {time.time() - time0}")	
